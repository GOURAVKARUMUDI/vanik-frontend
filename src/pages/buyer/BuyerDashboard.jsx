        ...
        <div style={pageStyle}>
            <div style={{ maxWidth: '900px', margin: '0 auto' }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    {/* Location Selection & Metrics Grid */}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1.5rem', marginBottom: '2.5rem' }}>
                        {/* Location Card */}
                        <div style={locationCard}>
                            <h3 style={{ fontSize: '1rem', fontWeight: 800, color: '#7C3E2F', marginBottom: '1rem' }}>📍 My Location</h3>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                <div>
                                    <label style={labelStyle}>Current Campus</label>
                                    <p style={valueStyle}>{user?.campus || 'Not Set'}</p>
                                </div>
                                <div>
                                    <label style={labelStyle}>City</label>
                                    <p style={valueStyle}>{user?.city || 'Not Set'}</p>
                                </div>
                                <button
                                    onClick={() => navigate('/complete-profile')}
                                    style={{ ...secondaryBtn, padding: '0.4rem 0.8rem', fontSize: '0.75rem', marginTop: '0.5rem' }}
                                >
                                    Change Location
                                </button>
                            </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
                            ...
                            <div style={metricCard}>
                                ...
                                <span style={metricLabel}>Received</span>
                            </div>
                        </div>
                    </div>
                                ...
                                >
                                    <div>
                                        ...
                                        <h3 style={{ margin: '0.5rem 0 0.25rem', color: '#1E1E1E', fontWeight: 700 }}>
                                            {order.product?.title || 'Purchase'}
                                        </h3>
                                        <p style={{ margin: 0, color: '#888', fontSize: '0.85rem' }}>
                                            {new Date(order.createdAt || Date.now()).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}
                                        </p>
                                    </div>
                                    <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                                        <div style={{ textAlign: 'right' }}>
                                            <p style={{ fontWeight: 900, fontSize: '1.5rem', color: '#7C3E2F', margin: 0 }}>₹{order.totalPrice}</p>
                                        ...
                                        {order.shippingAddress && (
                                            <div style={trackingBox}>
                                                <p style={trackingText}>🚚 {order.campusDetails || 'Pickup'}</p>
...
const metricCard = { background: '#fff', padding: '1.5rem', borderRadius: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', borderTop: '4px solid #7C3E2F' }
...
const locationCard = { background: '#fff', padding: '1.5rem', borderRadius: '1rem', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', borderLeft: '4px solid #D4AF37' }
const labelStyle = { fontSize: '0.65rem', fontWeight: 800, color: '#aaa', textTransform: 'uppercase', display: 'block' }
const valueStyle = { fontSize: '0.9rem', fontWeight: 700, color: '#444', margin: '0' }
